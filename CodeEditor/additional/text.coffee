...
text._append = append = (newOp, c) ->
  ...
  if newOp.length == 0
    ...
    if last.i? && c.i? and last.p <= c.p <= (last.p + last.i.length)
      newOp[newOp.length - 1] = {i:strInject(last.i, c.p - last.p, c.i), p:last.p, meta:c.meta}
    else if last.d? && c.d? and c.p <= last.p <= (c.p + c.d.length)
      newOp[newOp.length - 1] = {d:strInject(c.d, last.p - c.p, last.d), p:c.p, meta:c.meta}
    else
      newOp.push c
...
text._tc = transformComponent = (dest, c, otherC, side) ->
  ...
  if c.i?
    append dest, {i:c.i, p:transformPosition(c.p, otherC, side == 'right'), meta: c.meta}

  else # Delete
    if otherC.i? # delete vs insert
      s = c.d
      if c.p < otherC.p
        append dest, {d:s[...otherC.p - c.p], p:c.p, meta:c.meta}
        s = s[(otherC.p - c.p)..]
      if s != ''
        append dest, {d:s, p:c.p + otherC.i.length, meta:c.meta}

    else # Delete vs delete
      if c.p >= otherC.p + otherC.d.length
        append dest, {d:c.d, p:c.p - otherC.d.length, meta:c.meta}
      else if c.p + c.d.length <= otherC.p
        append dest, c
      else
        # They overlap somewhere.
        newC = {d:'', p:c.p, meta:c.meta}
        if c.p < otherC.p
...
invertComponent = (c) ->
  if c.i?
    {d:c.i, p:c.p, meta:c.meta}
  else
    {i:c.d, p:c.p, meta:c.meta}
